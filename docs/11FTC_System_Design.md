...
| `13-project-plan-trello.md` | Phased delivery plan and timeline, structured for Trello import. |
| `14-module-specifications.md` | Per-module contracts, invariants, and merge-gating tests. Read before implementing any module. |
...
- **Web Application** — Next.js. Ticket forms, queue view, employee management, dashboard. No business logic.
- **Application API** — NestJS. All business logic; the transaction boundary.
- **Sync Worker** — NestJS, second entrypoint, same codebase. Drains the outbox, talks to Sheets.
- **Supabase Postgres** — the system of record.
- **Redis** — wakes the worker. Triggers only.

### 2.1 Technology decisions (recorded)

| Layer | Choice | Why this and not the alternative |
|---|---|---|
| Frontend | Next.js | Given |
| API | **NestJS** | The component decomposition in `10-component.d2` maps onto NestJS modules and providers almost one-to-one — `TicketService`, `TicketNumberGenerator`, `EmployeeResolver` are providers; the §3.3 matrix is a `RolesGuard`. Fastify is lighter but loses the DI that mirrors the design. Laravel + Horizon has better queue tooling but breaks TypeScript alignment with Next.js. |
| DB access | **Drizzle**, session pooler (5432) | `supabase-js` speaks PostgREST and **cannot do multi-statement transactions** — which would break FR-31 outright. A persistent NestJS process can use the session pooler with a normal pool, so prepared statements work; the transaction-pooler constraints only apply to serverless. |
| Database | **Supabase Postgres** | Given. Plain Postgres — nothing in the design depends on Supabase-specific features. |
| Auth | **Supabase Auth**, verified via JWKS | The API verifies session JWTs locally against `/auth/v1/.well-known/jwks.json` using `jose`. No Supabase call per request. The endpoint is edge-cached ~10 min; do not cache longer or key rotation becomes painful. |
| Queue | **BullMQ + Redis** | Retries, backoff, concurrency, and Bull Board for the cost of a library import, given a long-running process is running anyway. **Constrained by the rule below.** |
| Sheets | `googleapis` + `google-auth-library` | Node-native; service-account JWT signing handled for you. |
| Cache | **None initially** | See §4. Redis is present for BullMQ, so adding cache later is free. Wire the infrastructure, skip the invalidation logic until a query is actually slow. |

**The rule that constrains BullMQ:** *the job triggers the work; the outbox decides what work exists.* The job payload carries nothing but "wake up and drain" — never the ticket data. A lost job must be harmless. Alongside the on-commit dispatch, run a **repeatable job every minute** as a sweeper, so any row missed by a dropped dispatch is picked up on the next tick. BullMQ's retries handle transient blips; `sync_outbox.attempts` and `status` remain the source of truth.

> **Why this is written down here.** The choice was made in conversation and lived nowhere. A stack decision that exists only in someone's memory is how `13-project-plan-trello.md` came to specify Supabase Cron for a system that uses BullMQ. Decisions go in the document or they drift.
...
| 9 | Sheet sort order undocumented | **Closed** | SRS FR-28 |

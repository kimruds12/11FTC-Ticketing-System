...
| FR | Requirement | Use case | Diagrams |
|---|---|---|---|
| FR-24 | One-way sync: DB → sheet | Synchronize to Sheet | DFD0 (no return flow), SYNC, C4 |
| FR-25 | Export using existing columns | Synchronize to Sheet | SYNC, SQ3 |
| FR-26 | Resolve FKs to names at export | Synchronize to Sheet | SQ3, CMP (`SheetRowProjector`) |
| FR-27 | Preserve newest-first display | Synchronize to Sheet | SYNC (`_raw` + QUERY), SQ3, CMP (`RowLocator`) |
| FR-28 | Sync failure must not block encoding | Synchronize to Sheet | SQ1, SQ3, SYNC, C4 |
| FR-29 | Idempotent writes | Synchronize to Sheet | SQ3, ERD (`row_key`) |
| FR-30 | Enqueue sync in the same transaction | Enqueue Synchronization «include» | SQ1, SQ2, ERD, CLS |
| FR-31 | Backfill historical rows | Backfill History | UC, SYNC |

### 2.5 Audit Logging (SRS §4.5)

| FR | Requirement | Use case | Diagrams |
|---|---|---|---|
| FR-32 | One entry per changed field | Write Audit Entry «include» | SQ2, ERD, DFD1 (6.0) |
| FR-33 | Written in the same transaction | Write Audit Entry «include» | SQ1, SQ2 |
| FR-34 | Immutable | Write Audit Entry «include» | ERD, CMP |
...
| Enqueue Synchronization | «include» — no actor | §4.4 FR-30, §9A |
...
| Configure Synchronization | Administrator | §4.4 FR-24 |
| View Dashboard | Administrator, **IT Staff (OPEN-2)** | §3.3, §4.3 |
| Generate Report | Administrator | §2 |
| Synchronize to Sheet | none — system-triggered | §4.4 FR-24–28 |
| Backfill History | Administrator | §4.4 FR-31 |
